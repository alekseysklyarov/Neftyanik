...
IF NOT EXISTS (
    ...
    WHERE [MigrationId] = N'20260729185604_InitialCleanSchema'
)
BEGIN
...
BEGIN
    ...
    EXEC(N'INSERT INTO [ExpenseCategories] ([Id], [Description], [IsActive], [Name])
...
BEGIN
    ...
    EXEC(N'INSERT INTO [MembershipFeeRates] ([Id], [AmountPerPlot], [CreatedAt], [DueDate], [IsActive], [Year])
...
BEGIN
    EXEC(N'CREATE UNIQUE INDEX [IX_AssociationElectricityReadings_IsInitialReading] ON [AssociationElectricityReadings] ([IsInitialReading]) WHERE [IsInitialReading] = 1');
END;
GO
...
BEGIN
    EXEC(N'CREATE UNIQUE INDEX [IX_ChargeTypes_Code] ON [ChargeTypes] ([Code]) WHERE [Code] IS NOT NULL');
END;
GO
...
BEGIN
    EXEC(N'CREATE UNIQUE INDEX [IX_ChargeTypes_IsDefault] ON [ChargeTypes] ([IsDefault]) WHERE [IsDefault] = 1 AND [IsActive] = 1');
END;
GO
...
BEGIN
    EXEC(N'CREATE UNIQUE INDEX [IX_Expenses_AssociationElectricityReadingId] ON [Expenses] ([AssociationElectricityReadingId]) WHERE [AssociationElectricityReadingId] IS NOT NULL');
END;
GO
...
BEGIN
    EXEC(N'CREATE UNIQUE INDEX [IX_MemberElectricityReadings_ChargeId] ON [MemberElectricityReadings] ([ChargeId]) WHERE [ChargeId] IS NOT NULL');
END;
GO
...
BEGIN
    EXEC(N'CREATE UNIQUE INDEX [IX_MemberElectricityReadings_MemberElectricityMeterId_IsInitialReading] ON [MemberElectricityReadings] ([MemberElectricityMeterId], [IsInitialReading]) WHERE [IsInitialReading] = 1');
END;
GO
...
BEGIN
    EXEC(N'CREATE UNIQUE INDEX [IX_PlotOwnerships_PlotId] ON [PlotOwnerships] ([PlotId]) WHERE [ValidTo] IS NULL');
END;
GO
