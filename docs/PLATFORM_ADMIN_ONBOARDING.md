...
| --- | --- |
| First administrator on the existing tenant-only installation, after the owner's history check and explicit authorization | `dotnet ./Neftyanik.Portal.Web.dll initialize-first-platform-admin-for-existing-installation` |
...
For first provisioning on an existing installation, the project owner may make the decision personally: no independent approver or approval/change reference is required. Deployment approval and the separate SSH password-recovery procedure are unchanged.
...
## Owner-confirmed legacy initialization: existing tenant-only installation

The dedicated command is `initialize-first-platform-admin-for-existing-installation`. It is **not** a recovery command, a second bootstrap, a `--force` override, or a Web endpoint. Its application service is registered only in this CLI process, not in the normal Web host. Tenant Administrator membership supplies no authority to run it. The interactive guard and owner declaration are safety/audit controls, **not authorization credentials**; controlled OS/deployment execution and database access are the authorization boundary. The declaration is an assertion by the operator, not independent proof of project ownership. Restrict executable/configuration access, shell access and database credentials to authorized operators. A compromised OS/database administrator remains outside the application's protection.

### Eligibility and the owner's decision, before execution

1. The project owner checks the intended installation using deployment history, old release versions, audit/change records and protected historical backups. **Absence of current platform accounts, roles or claims does not prove that an administrator never existed.** The existing `LegacyReviewRequired` status still records a need to check history, not permission to create an account; its name does not require an independent approver.
2. The owner personally confirms that this will be the first `PlatformAdministrator` account and authorizes its creation. No independent agreement, `REVIEWED` response or approval/change reference is requested. If evidence is incomplete, contradictory, or shows previous provisioning, stop and use the emergency-review process below; the owner's declaration cannot override historical ineligibility or the permanent marker.
...
2. Displays: **I am the project owner. This is the first PlatformAdministrator account, and I authorize its creation.** Type exactly `CREATE FIRST PLATFORMADMINISTRATOR` to confirm. Empty input, EOF, `REVIEWED`, `YES`, different case or surrounding spaces are rejected before credential entry or provisioning. No approval/change reference is collected.
...
4. Captures the executing OS domain/user identity (maximum 256 characters), not an identity supplied by a Web form. The existing `PlatformBootstrapStates` audit stores `Reason = Owner attestation: first PlatformAdministrator; creation authorized`, that actual OS identity in `OperatorIdentity`, `InitializedAtUtc`, and the created `InitializedUserId` (maximum 450 characters, no foreign key). `ApprovalReference` remains `null`: no external approval is claimed or fabricated. In a container or under a service account, the recorded identity is that process's OS account, not verified human identity; retain separate operator/session attribution if needed. Audit fields must never contain passwords, reset tokens or SMTP secrets. Existing audit records are preserved; no schema change or new migration is needed.

### Transaction and permanent protection

The service refuses requests without explicit owner confirmation. Inside a serializable transaction, it acquires the transaction-owned exclusive `DachaHub.FirstPlatformAdministrator` lock, shared with ordinary bootstrap and migration. It requires exactly one marker with ID 1, disposition `LegacyReviewRequired`, a valid classification timestamp, and no existing initialization/operator/approval metadata. Existing approval metadata still signals prior or inconsistent initialization and is not erased. The service also requires existing Identity users and rechecks all available platform evidence. Missing/multiple/unknown/inconsistent markers, a pristine store, or `Consumed` are refused. If surviving platform evidence is discovered, the marker is permanently sealed as `Consumed`, without modifying Identity or tenant records, so deleting the evidence afterwards cannot make the command eligible.
...
The approved `reset-platform-admin-password` command recovers credentials for an existing eligible platform account even when email is unavailable. It is **not** a general identity/role-restoration command. Owner-confirmed legacy initialization is only for a historically verified first administrator and cannot be used to repair or replace a previous one. Deleted accounts/roles, intentional access revocation or disabled/locked accounts require separate incident review; this reset command will not bypass those states. If historical eligibility cannot be established, stop and preserve evidence; the owner's declaration alone cannot override ineligibility. Do not remove `PlatformBootstrapStates`, edit old claims, change `Consumed` to `LegacyReviewRequired`, rerun either provisioning command, silently create a replacement administrator, or restore a whole production database merely to reset one account.
